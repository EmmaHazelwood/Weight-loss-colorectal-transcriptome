sink('biomartScriptResultstransverse.txt')
library(data.table)
library(dplyr)
library(tidyr)
...
SNPs<-distinct(SNPs)
fwrite(SNPs,"SNPsformyvarianttransverse.csv")
SNPs<-fread("SNPsformyvarianttransverse.csv")
...
write.table(results, file = "biomartResultstransverse.txt", sep = "\t",
            row.names = F)
