sink('biomartScriptResultsSigmoid.txt')
library(data.table)
library(dplyr)
library(tidyr)
...
SNPs<-distinct(SNPs)
fwrite(SNPs,"SNPsformyvariantsigmoid.csv")
SNPs<-fread("SNPsformyvariantsigmoid.csv")
...
write.table(results, file = "biomartResultssigmoid.txt", sep = "\t",
            row.names = F)
